...
describe('FavoriteLibraryNavigation contract', () => {
  ...
  it('keeps a portal opened during first account binding before passive effects run', async () => {
    const props = {
      groups: [{ id: 'workspace', label: 'workspace', items: [{ id: 'folder:one', label: 'one', count: 1, managed: true }] }],
      collapsedGroups: {},
      selectedId: 'folder:one',
    ...
    function NavigationOpenedOnFirstAccountBinding() {
      const [uid, setUid] = useState('')
      useLayoutEffect(() => {
        if (!uid) setUid('100')
        else screen.getByRole('button', { name: 'one 菜单' }).click()
      }, [uid])
    ...
    try {
      ...
      await act(async () => root!.render(<NavigationOpenedOnFirstAccountBinding />))
