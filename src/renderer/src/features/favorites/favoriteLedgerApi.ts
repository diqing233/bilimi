  ...
  return `
    ...
    const remoteFolderCandidates = (ledger, folders) => {
      ...
      const pendingRemoteFolderId = ledger?.pendingRemoteBinding
        ...
        : '';
      const deduplicatedCandidates = candidates.filter((candidate, index, entries) =>
        entries.findIndex((entry) => entry.id === candidate.id) === index);
      return pendingRemoteFolderId
        ? deduplicatedCandidates.filter((candidate) => candidate.id === pendingRemoteFolderId)
    ...
    const syncLedgerFolderIds = (ledgers, folders, selectedRemoteFolderIds = {}, confirmCreateAndBind = false) => {
      ...
      return ledgers.filter((ledger) => {
        ...
        const selectedFolder = selectedRemoteFolderId && candidates.some((candidate) => candidate.id === selectedRemoteFolderId)
          ...
          : null;
        // An explicit candidate selection is the user's confirmation to hand
    ...
    const collectRemoteObservations = (
      ledgers,
      folders,
      ...
      for (const ledger of nextLedgers) {
        if (isPureRemoteObservationDraft(ledger) || ledger.bindingState !== 'bound') continue;
      ...
      const knownRemoteFolderIds = new Set([
        ...nextLedgers.flatMap(ledgerRemoteFolderIds),
        ...(Array.isArray(remoteDraftKnownFolderIds) ? remoteDraftKnownFolderIds : [])
          .map((folderId) => String(folderId || '').trim())
          .filter(Boolean)
      ]);
      ...
      const localRebindTitles = new Set(nextLedgers
        .filter((ledger) => isSavedOrdinaryRule(ledger))
      ...
      for (const folder of folders) {
        ...
        if (!folderId || dismissedIds.has(folderId) || boundFolderIds.has(folderId) ||
          confirmedDeletedFolderIds.has(folderId) || knownRemoteFolderIds.has(folderId) ||
        ...
        remoteObservations.push({ folderId, title: displayName, memberCount: videoCount });
      }
      return { ledgers: nextLedgers, remoteObservations };
    };
    const projectRemoteOnlyDrafts = (
      ...
      const projection = collectRemoteObservations(
        ledgers,
        folders,
        dismissedRemoteFolderIds,
        remoteDraftKnownFolderIds,
      ...
      const dismissedRemoteFolderIdSet = new Set((Array.isArray(dismissedRemoteFolderIds) ? dismissedRemoteFolderIds : [])
        ...
        .filter(Boolean));
      return {
        ledgers: projection.ledgers,
        remoteOnlyDraftLedgerIds: [],
        remoteObservations: projection.remoteObservations.filter((observation) => !dismissedRemoteFolderIdSet.has(observation.folderId))
      };
    };
  `
}
...
export function buildFavoriteLedgerStatusScript(
  ...
  return `
    (async () => {
      ...
      const remoteDraftProjection = {
        ...discoveredRemoteDraftProjection,
        remoteObservations: payload.includeRemoteOnlyDrafts !== false
      ...
      return {
        ...
        unboundCandidates,
        remoteOnlyDraftLedgerIds,
        remoteObservations: remoteDraftProjection.remoteObservations,
  ...
  return `
    (async () => {
      ...
      const projectRemoteDrafts = (sourceLedgers, sourceFolders) => {
        ...
        const projection = projectRemoteOnlyDrafts(
          synchronizedLedgers,
          sourceFolders,
        ...
        return {
          ...projection,
          remoteObservations: payload.options?.includeRemoteOnlyDrafts !== false
            ? projection.remoteObservations
            : []
        };
      };
      ...
      if (staleFormalBindingLedgerIds.length) {
        return {
          ...
          missingTargets: staleFormalBindingLedgerIds,
          backupConflictLedgerIds: [],
          unboundLedgerIds: [],
          remoteOnlyDraftLedgerIds: [],
      ...
      const missingTargets = nextLedgers.filter((ledger) => ledger.enabled && !isPureRemoteObservationDraft(ledger) && ledger.bindingState !== 'bound').map((ledger) => ledger.id);
