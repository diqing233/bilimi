      ...
      <div className="favorite-library__layout favorite-library__workspace" data-embedded-layout={embedded || undefined} data-footer-split="true">
        ...
        <FavoriteLibraryDetail title={detail?.title} collapsed={Boolean(detail && !detailOpen)} onRestore={() => setDetailOpen(true)} onCollapse={() => setDetailOpen(false)}>
          {detail ? <>
            ...
            })}>同步B站位置</button><button type="button" className="favorite-library__inline-action" onClick={() => void runAction(async () => {
              const api = window.bilimiDesktop
            ...
            <section><h3>{text.archive}</h3><p>{detailSnapshot?.archive.status ?? text.noArchive}</p>{detailSnapshot?.archive.versionCount ? <small>{`${detailSnapshot.archive.versionCount} 个版本`}</small> : null}{detailSnapshot?.archive.status === '已入档' && accountMid ? <><button type="button" className="favorite-library__inline-action" disabled={detailSnapshot.transcription.status !== '转写完成'} onClick={() => void runDetailAction(openArchiveDetail)}>查看档案详情</button><button type="button" onClick={() => void runDetailAction(async () => {
              const api = window.bilimiDesktop as typeof window.bilimiDesktop & FavoriteLibraryDesktopExtensions
