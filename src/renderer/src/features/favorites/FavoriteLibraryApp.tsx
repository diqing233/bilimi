...
export function FavoriteLibraryApp({
  ...
  const openSelectedPlacementPicker = (mode: 'replace' | 'copy' | 'move' = 'replace', aids = selectedAids) => {
    setPlacementDraftFolderIds([])
    setPlacementPickerBatch(true)
    setPlacementPickerMode(mode)
    setPlacementPickerAids([...aids])
    setPlacementSyncRequested(false)
    setPlacementPickerOpen(true)
  }
      ...
      if (placementPickerBatch) {
        await runAction(async () => {
          ...
          if (placementPickerMode === 'copy') {
            ...
            return api.copyFavoriteLibrarySelection(accountMid, placementPickerAids, placementDraftFolderIds, summary.revision, operationSource(placementPickerAids))
          }
          if (placementPickerMode === 'move') {
            if (!currentLogicalFolderId || !api?.moveFavoriteLibrarySelection) throw new Error(text.unavailable)
            return api.moveFavoriteLibrarySelection(accountMid, placementPickerAids, currentLogicalFolderId, placementDraftFolderIds, summary.revision, operationSource(placementPickerAids))
          }
  ...
  const deleteSelectedFromLibrary = async () => {
    ...
    await api.deleteFavoriteLibrarySelection(accountMid, selectedAids, summary.revision, operationSource(selectedAids))
    setSelectedAids([])
  ...
  const sourceEligibility = useMemo(() => determineFavoriteOperationEligibility({
    ...
    aids: selectedAids,
    folders,
    aidScopeKinds: Object.fromEntries(rows.map((row) => {
      const sourceFolder = row.folderIds.map((folderId) => folders.find((candidate) => candidate.id === folderId))
        .find((folder) => folder?.kind === 'bilibili')
      return sourceFolder ? [row.aid, sourceFolder.remoteFolderId === '1' ? 'bilibili-default' : 'bilibili-user-folder'] : [row.aid, undefined]
    }).filter(([, scopeKind]) => scopeKind))
  }), [currentFolderId, folders, rows, scope.kind, selectedAids])
  const operationSource = useCallback((requestedAids: number[]): FavoriteLibraryOperationSource => currentFolderId
    ...
    : {
      ...
      }, [currentFolderId, sourceEligibility])
      ...
      <div className="favorite-library__layout favorite-library__workspace" data-embedded-layout={embedded || undefined} data-footer-split="true">
        ...
        <section className="favorite-library__results" aria-label={text.results}>
          <FavoriteLibraryToolbar pageCount={rows.length} selectedCount={selectedAids.length} allCurrentPageSelected={allCurrentPageSelected} onTogglePage={toggleCurrentPage} batchDisabled={!eligibleSelectedAids.length} allowedActions={[...batchAllowedActions]} searchQuery={searchQuery} filter={rowFilter} sort={rowSort} onSearchChange={(query) => {
            ...
            if (!actionAids.length) return
            if (action === 'copy' || action === 'move') openSelectedPlacementPicker(action, actionAids)
            if (action === 'refresh') void runAction(async () => {
              const api = window.bilimiDesktop
              if (!api?.syncFavoriteLibrarySelection || !accountMid) throw new Error(text.unavailable)
              return api.syncFavoriteLibrarySelection(accountMid, { kind: 'aids', aids: actionAids })
            })
            if (action === 'transcribe') void runAction(async () => {
              ...
              return api.enqueueFavoriteLibraryTranscription(accountMid, { aids: actionAids })
            })
            if (action === 'delete-local') setBatchLocalDeleteConfirmationOpen(true)
            if (action === 'unfavorite-remote' && accountMid && summary) void (async () => {
              const api = window.bilimiDesktop
              const preview = await api.previewFavoriteLibraryRemoteUnfavoriteOperation?.(accountMid, selectedAids, summary.revision, operationSource(selectedAids)) as {
        ...
        <FavoriteLibraryDetail selected={Boolean(detail)} collapsed={Boolean(detail && !detailOpen)} onRestore={() => setDetailOpen(true)} onCollapse={() => setDetailOpen(false)}>
          {detail ? <>
            ...
            <div className="favorite-library__detail-heading"><h2>{detail.title}</h2><span className="favorite-library__detail-heading-actions">{accountMid ? <button type="button" className="favorite-library__open-video" onClick={() => void runDetailAction(async () => {
            ...
            <p>{detail.author ?? text.unknownAuthor} · {detailSnapshot?.video.bvid ?? `AV${detail.aid}`} · {detailSnapshot?.video.cid ? `分P CID：${detailSnapshot.video.cid}` : `AV${detail.aid}`}</p>
            ...
            })}>同步B站位置</button><button type="button" className="favorite-library__inline-action" onClick={() => void runDetailAction(() => setLocalPlacement([]))}>移出所有本地仓库</button>{detailSnapshot?.position ? <button type="button" className="favorite-library__inline-action" disabled={!detailSnapshot.position.remoteObservedPhysicalFolderIds.length || ['failed', 'result-unknown', 'needs-review', 'syncing'].includes(detailSnapshot.position.state)} onClick={() => void runDetailAction(adoptRemotePlacement)}>采用B站位置</button> : null}</> : null}</div></section>
            <section><h3>音频与档案</h3><p>{text.transcriptionState}：{detailSnapshot?.transcription.status ?? (detail.pendingStates.includes('continuation') ? '等待处理' : '暂无转写任务')}</p><p>{text.archive}：{detailSnapshot?.archive.status ?? text.noArchive}{detailSnapshot?.archive.versionCount ? ` · ${detailSnapshot.archive.versionCount} 个版本` : ''}</p><div className="favorite-library__detail-action-row"><button type="button" className="favorite-library__inline-action" onClick={() => void runAction(async () => {
              const api = window.bilimiDesktop
            ...
            })}>单独转写音频</button>{detailSnapshot?.archive.status === '已入档' && accountMid ? <><button type="button" className="favorite-library__inline-action" disabled={detailSnapshot.transcription.status !== '转写完成'} onClick={() => void runDetailAction(openArchiveDetail)}>查看档案详情</button><button type="button" onClick={() => void runDetailAction(async () => {
              const api = window.bilimiDesktop as typeof window.bilimiDesktop & FavoriteLibraryDesktopExtensions
