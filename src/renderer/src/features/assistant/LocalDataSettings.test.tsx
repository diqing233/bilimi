...
describe('LocalDataSettings', () => {
  ...
  it('groups local storage, migration, and destructive controls without exposing every account by default', () => {
    ...
    expect(screen.getByRole('heading', { name: '本地数据' })).toHaveClass('local-data-settings__heading')
    expect(screen.getByRole('heading', { name: '数据迁移' })).toHaveClass('local-data-settings__heading')
    expect(screen.getByRole('heading', { name: '管理数据' })).toHaveClass('local-data-settings__heading')
    const managementToggle = screen.getByRole('button', { name: '管理数据' })
    expect(managementToggle).toHaveClass('local-data-settings__section-toggle')
    expect(managementToggle).toHaveAttribute('aria-expanded', 'false')
    expect(managementToggle.querySelector('.local-data-settings__section-chevron')).toBeInTheDocument()
    expect(screen.queryByRole('heading', { name: '危险操作' })).not.toBeInTheDocument()
    expect(screen.getByText('本机保存 2 个账号的数据')).toBeInTheDocument()
    expect(screen.queryByRole('listitem', { name: '200' })).not.toBeInTheDocument()
