...
describe('ControlledFavoriteLedgerPanel', () => {
  ...
  it('keeps recommendation cancellation separate from an explicit upper-rule deletion', async () => {
    const workspace = {
    ...
    expect(window.bilimiDesktop?.commandOldFavoriteWorkspaceV1).not.toHaveBeenCalledWith('100', {
  ...
  it('keeps a promoted recommendation draft and cancels its exact lower adoption when its upper card is unchecked', async () => {
    const preview = {
      ...
      recommendations: {
        candidates: [{ id: 'promoted-honker', displayName: 'bilimi·honker233', kind: 'author' as const, keywords: ['honker233'], count: 1, reason: '推荐 UP' }],
        adoptedCandidateIds: ['promoted-honker']
      },
      history: { cursor: 0, length: 0 }
    }
    ...
    render(<ControlledFavoriteLedgerPanel currentAccountMid="100" ledgers={[{
      id: 'promoted-honker', displayName: 'bilimi·honker233', keywords: ['honker233'], ruleType: 'author', enabled: true,
      priority: 10_000, syncState: 'local-draft', ruleOrigin: 'recommendation-draft', isDefault: false
  ...
  it('commits a linked upper-rule selection before merging its durable enabled state', async () => {
    const preview = {
      ...
      classifications: {}, excludedLedgerIds: [] as string[],
      recommendations: {
        candidates: [{ id: 'saved-honker', displayName: 'bilimi·honker233', kind: 'author' as const, keywords: ['honker233'], count: 1, reason: '已保存规则' }],
    ...
    const command = vi.fn((_accountMid: string, request: { type: string; ledgerIds?: string[] }) => {
      ...
      return Promise.resolve({
        ...
        excludedLedgerIds: request.type === 'set-round-excluded-ledger-ids' ? request.ledgerIds ?? [] : preview.excludedLedgerIds,
        recommendations: request.type === 'set-recommended-candidates'
          ? { ...preview.recommendations, adoptedCandidateIds: [] }
          : preview.recommendations
      })
    })
    const saveEnabled = vi.fn((ledgerId: string, enabled: boolean, options?: { mergeFavoriteRuleHistory?: true }) => {
      order.push(`save:${ledgerId}:${String(enabled)}:${options?.mergeFavoriteRuleHistory === true}`)
    ...
    render(<ControlledFavoriteLedgerPanel currentAccountMid="100" ledgers={[{
      id: 'saved-honker', displayName: 'bilimi·honker233', keywords: ['honker233'], ruleType: 'author', enabled: true,
  ...
  it('pauses and persists a preview draft before closing the guide', async () => {
    const preview = {
      version: 1 as const, accountMid: '100', workspaceId: 'workspace-100', status: 'previewing' as const,
      mode: 'incremental' as const, segmentSize: 2000, hasMultipleSegments: false,
      scan: { phase: 'complete' as const, failureCount: 0 }, sourceFolders: [], continuationCount: 0,
    ...
    expect(prepareRecovery).toHaveBeenCalledTimes(2)
    expect(prepareRecovery).toHaveBeenLastCalledWith('100')
  ...
  it('keeps the draft open and reports a pause failure instead of pretending it closed', async () => {
    const preview = {
      version: 1 as const, accountMid: '100', workspaceId: 'workspace-100', status: 'previewing' as const,
      mode: 'incremental' as const, segmentSize: 2000, hasMultipleSegments: false,
      scan: { phase: 'complete' as const, failureCount: 0 }, sourceFolders: [], continuationCount: 0,
    ...
    const prepareRecovery = vi.fn()
      .mockResolvedValueOnce(null)
      .mockRejectedValueOnce(new Error('扫描检查点未能保存。'))
    window.bilimiDesktop = {
      openOldFavoriteWorkspaceV1: vi.fn().mockResolvedValue(preview),
      prepareOldFavoriteWorkspaceRecoveryV1: prepareRecovery,
      commandOldFavoriteWorkspaceV1: vi.fn().mockResolvedValue(null)
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent('扫描检查点未能保存。')
    expect(prepareRecovery).toHaveBeenCalledTimes(2)
    expect(prepareRecovery).toHaveBeenLastCalledWith('100')
    expect(screen.getByRole('region', { name: '整理收藏向导' })).toBeInTheDocument()
  })
  ...
  it('refreshes the active archive preview after a saved rule reclassifies the current batch', async () => {
    const preview = {
      version: 1 as const, accountMid: '100', workspaceId: 'workspace-100', status: 'previewing' as const,
      mode: 'incremental' as const, segmentSize: 2_000, hasMultipleSegments: false,
      scan: { phase: 'complete' as const, failureCount: 0 }, continuationCount: 0,
      ...
      currentSegment: { id: 'segment-1', aids: [1], items: [{ aid: 1, title: 'Honker video', sourceFolderIds: ['source'] }] },
      classifications: { '1': { aid: 1, targetLedgerIds: ['game'], source: 'system-high' as const } },
      recommendations: { candidates: [], adoptedCandidateIds: [] }, history: { cursor: 1, length: 1 }
    }
    const reclassified = {
      ...preview,
      classifications: { '1': { aid: 1, targetLedgerIds: ['custom-honker'], source: 'system-high' as const } }
    }
    const open = vi.fn().mockResolvedValue(preview)
    const command = vi.fn().mockResolvedValue(reclassified)
    ...
    render(<ControlledFavoriteLedgerPanel currentAccountMid="100" missingLedgerIds={[]}
      ...
      ledgers={[{ id: 'custom-honker', displayName: 'bilimi·honker233', keywords: ['honker233'], ruleType: 'author', enabled: true, priority: 0, isDefault: false }]} />)

    await openPersistedWorkspaceGuide()
    await waitFor(() => expect(open).toHaveBeenCalled())
    fireEvent.click(screen.getByRole('button', { name: 'honker233' }))
    fireEvent.change(screen.getByRole('textbox', { name: 'UP 名字' }), { target: { value: 'honker233 Honker' } })
    fireEvent.click(screen.getByRole('button', { name: '保存' }))
    ...
    expect(await screen.findByRole('group', { name: 'bilimi·honker233 1 条' })).toBeInTheDocument()
  })

  it('reclassifies the actual preview workspace after a saved rule when the parent snapshot is absent', async () => {
    const preview = {
      version: 1 as const, accountMid: '100', workspaceId: 'workspace-100', status: 'previewing' as const,
      mode: 'incremental' as const, segmentSize: 2_000, hasMultipleSegments: false,
    ...
    const reclassified = {
      ...preview,
      classifications: { '1': { aid: 1, targetLedgerIds: ['custom-music'], source: 'system-high' as const } }
    }
    const command = vi.fn((_: string, request: { type: string }) => request.type === 'save-draft-ledger-rule'
      ? Promise.resolve(preview)
      : Promise.resolve(reclassified))
    window.bilimiDesktop = {
      openOldFavoriteWorkspaceV1: vi.fn().mockResolvedValue(preview),
      commandOldFavoriteWorkspaceV1: command
    } as unknown as typeof window.bilimiDesktop
    render(<ControlledFavoriteLedgerPanel currentAccountMid="100" ledgers={[
      { id: 'custom-music', displayName: 'bilimi·音乐', keywords: ['旋律'], ruleType: 'keyword', enabled: true, priority: 0, isDefault: false }
    ]} missingLedgerIds={[]} onEnsureLedgers={vi.fn()} onSaveLedgers={vi.fn().mockResolvedValue(undefined)} />)
  ...
  it('persists a matched saved top rule while removing it from the active round', async () => {
    const preview = {
      version: 1 as const, accountMid: '100', workspaceId: 'workspace-100', status: 'previewing' as const,
      mode: 'incremental' as const, segmentSize: 2000, hasMultipleSegments: false,
      scan: { phase: 'completed' as const, failureCount: 0 }, sourceFolders: [], continuationCount: 0,
  ...
  it('keeps a saved high-priority rule and its linked recommendation when the upper card is unchecked', async () => {
    ...
    expect(command).toHaveBeenCalledWith('100', {
      type: 'set-round-excluded-ledger-ids', ledgerIds: ['custom-honker'], mergeFavoriteRuleHistory: true
    })
  ...
  it('restores a disabled recommendation draft when its linked lower recommendation is checked again', async () => {
    ...
    expect(command).toHaveBeenCalledWith('100', {
      type: 'set-round-excluded-ledger-ids', ledgerIds: [], mergeFavoriteRuleHistory: true
    })
    await waitFor(() => expect(screen.getByRole('button', { name: '移出同步 bilimi·honker233' })).toBeInTheDocument())
  })
  ...
  it('persists a saved ledger cancellation while updating the active round projection', async () => {
    const preview = {
      version: 1 as const, accountMid: '100', workspaceId: 'workspace-100', status: 'previewing' as const,
      mode: 'incremental' as const, segmentSize: 2000, hasMultipleSegments: false,
      scan: { phase: 'completed' as const, failureCount: 0 }, sourceFolders: [], continuationCount: 0,
    ...
    const command = vi.fn().mockResolvedValue({ ...preview, excludedLedgerIds: ['custom-music'] })
  ...
  it('persists every saved cancellation while committing one active-round projection', async () => {
    ...
    const command = vi.fn().mockResolvedValue({ ...preview, excludedLedgerIds: ['custom-music', 'custom-tech'] })
    const saveEnabled = vi.fn()
    window.bilimiDesktop = {
      openOldFavoriteWorkspaceV1: vi.fn().mockResolvedValue(preview),
      commandOldFavoriteWorkspaceV1: command
    ...
    render(<ControlledFavoriteLedgerPanel currentAccountMid="100" ledgers={[
      { id: 'custom-music', displayName: 'bilimi·音乐', keywords: ['旋律'], ruleType: 'keyword', enabled: true, priority: 0, isDefault: false },
      { id: 'custom-tech', displayName: 'bilimi·科技', keywords: ['芯片'], ruleType: 'keyword', enabled: true, priority: 10, isDefault: false }
    ]} missingLedgerIds={[]} onEnsureLedgers={vi.fn()} onSaveLedgers={vi.fn()} onSaveLedgerEnabled={saveEnabled} />)

    await screen.findByRole('button', { name: '取消全选' })
    fireEvent.click(screen.getByRole('button', { name: '取消全选' }))

    await waitFor(() => expect(command).toHaveBeenCalledWith('100', {
      type: 'set-round-excluded-ledger-ids', ledgerIds: ['custom-music', 'custom-tech']
    }))
  ...
  it('cancels every saved rule and its linked recommendation from the saved-rule bulk selection', async () => {
    const preview = {
      ...
      segments: [], currentSegment: null, classifications: {},
      recommendations: {
        candidates: [{ id: 'custom-author', displayName: 'bilimi·UP', kind: 'author' as const, keywords: ['UP'], count: 1, reason: '已保存规则' }],
        adoptedCandidateIds: ['custom-author']
      },
      history: { cursor: 0, length: 0, entries: [] }
    }
    const command = vi.fn().mockResolvedValue({ ...preview, excludedLedgerIds: ['custom-author', 'custom-music'] })
    const saveEnabled = vi.fn()
    window.bilimiDesktop = {
      openOldFavoriteWorkspaceV1: vi.fn().mockResolvedValue(preview),
      commandOldFavoriteWorkspaceV1: command
    ...
    render(<ControlledFavoriteLedgerPanel currentAccountMid="100" ledgers={[
      { id: 'custom-author', displayName: 'bilimi·UP', keywords: ['UP'], ruleType: 'author', enabled: true, priority: 0, isDefault: false },
      { id: 'custom-music', displayName: 'bilimi·音乐', keywords: ['旋律'], ruleType: 'keyword', enabled: true, priority: 10, isDefault: false }
    ]} missingLedgerIds={[]} onEnsureLedgers={vi.fn()} onSaveLedgers={vi.fn()} onSaveLedgerEnabled={saveEnabled} />)
  ...
  it('includes a recommendation-linked saved rule in the saved-rule exclusion batch', async () => {
    const preview = {
      ...
      scan: { phase: 'completed' as const, failureCount: 0 }, sourceFolders: [], continuationCount: 0,
      segments: [], currentSegment: null, classifications: {},
      recommendations: { candidates: [{ id: 'custom-author', displayName: 'bilimi·UP', kind: 'author' as const, count: 1, reason: 'saved' as const }], adoptedCandidateIds: ['custom-author'] },
      history: { cursor: 0, length: 0, entries: [] }
    }
    const command = vi.fn().mockResolvedValue({ ...preview, excludedLedgerIds: ['custom-author', 'custom-music'] })
    window.bilimiDesktop = {
    ...
    render(<ControlledFavoriteLedgerPanel currentAccountMid="100" ledgers={[
      { id: 'custom-music', displayName: 'bilimi·音乐', keywords: ['旋律'], ruleType: 'keyword', enabled: true, priority: 0, isDefault: false },
      { id: 'custom-author', displayName: 'bilimi·UP', keywords: ['UP'], ruleType: 'author', enabled: true, priority: 10, isDefault: false }
    ]} missingLedgerIds={[]} onEnsureLedgers={vi.fn()} onSaveLedgers={vi.fn()} />)
  ...
  it('shows an explicit recovery choice before loading an unfinished persisted workspace', async () => {
    ...
    const command = vi.fn(({ type, candidateIds }: { type: string; candidateIds?: string[] }) => Promise.resolve({
      ...preview,
      recommendations: {
        ...preview.recommendations,
        adoptedCandidateIds: type === 'set-recommended-candidates' ? (candidateIds ?? []) : preview.recommendations.adoptedCandidateIds
      }
    }))
    window.bilimiDesktop = {
      openOldFavoriteWorkspaceV1: vi.fn().mockResolvedValue(preview),
  ...
  it('shows recovery choices only after the silent background preparation completes', async () => {
    ...
    expect(screen.queryByText('正在暂停并保存进度…')).not.toBeInTheDocument()
  ...
  it('restores the persisted recommendation selection instead of re-adopting a globally enabled ledger', async () => {
    ...
    const command = vi.fn((_accountMid: string, input: { type: string; candidateIds?: string[] }) => Promise.resolve({
      ...
      recommendations: {
        ...preview.recommendations,
        adoptedCandidateIds: input.type === 'set-recommended-candidates'
          ? (input.candidateIds ?? [])
          : preview.recommendations.adoptedCandidateIds
      }
    }))
  ...
  it('keeps the recovery dialog open when abandoning fails', async () => {
    ...
    fireEvent.click(await screen.findByRole('button', { name: '整理收藏' }))
    const dialog = await screen.findByRole('dialog', { name: '整理收藏' })
    fireEvent.click(within(dialog).getByRole('button', { name: '放弃本轮整理' }))

    await waitFor(() => expect(command).toHaveBeenCalledWith('100', { type: 'abandon-current-workspace' }))
    expect(await within(dialog).findByRole('alert')).toHaveTextContent('扫描草稿仍在运行，无法放弃。')
    expect(screen.getByRole('dialog', { name: '整理收藏' })).toBeInTheDocument()
  })
  ...
  it('rechecks logical backup gaps before provisioning a required Bilibili shard', async () => {
    ...
    const stillMissingPreflight = { ...initialPreflight, requiredPhysicalShards: [] }
    const preflight = vi.fn()
      .mockResolvedValueOnce(initialPreflight)
      .mockResolvedValueOnce(initialPreflight)
      .mockResolvedValueOnce(stillMissingPreflight)
    const provision = vi.fn().mockResolvedValue(initialPreflight)
    const command = vi.fn().mockResolvedValue(preview)
    ...
    const confirmation = await screen.findByRole('dialog', { name: '同步前备册确认' })
    expect(confirmation).toHaveTextContent('收藏夹：bilimi·honker233（未备册）')
    expect(confirmation).toHaveTextContent('新增分册：bilimi·游戏专区·2（本轮需容纳 1 条新归属）')
    expect(command).not.toHaveBeenCalled()
    fireEvent.click(within(confirmation).getByRole('button', { name: '确认备册并继续' }))
    ...
    await waitFor(() => expect(screen.getAllByRole('alert').some((alert) => alert.textContent?.includes('备册尚未完成'))).toBe(true))
  ...
  it('refreshes the authoritative projection after adopting a discovered capacity shard in the same confirmation', async () => {
    const preview = {
      version: 1 as const, accountMid: '100', workspaceId: 'workspace-100', status: 'previewing' as const,
      mode: 'incremental' as const, segmentSize: 2000, hasMultipleSegments: false,
      scan: { phase: 'complete' as const, failureCount: 0 }, continuationCount: 0, sourceFolders: [],
    ...
    const initialPreflight = {
      ...
      missingLedgers: [{ logicalLedgerId: 'honker233', logicalTitle: 'bilimi·honker233', reason: 'unbacked' as const }],
      requiredPhysicalShards: [{
        logicalLedgerId: 'game', logicalTitle: 'bilimi·游戏专区', shardNumber: 2, requiredAssignmentCount: 1,
        bindingCandidates: [{ remoteFolderId: 'game-2', remoteTitle: 'bilimi·游戏专区·2', memberCount: 3 }]
      }]
    }
    ...
    window.bilimiDesktop = {
      openOldFavoriteWorkspaceV1: vi.fn().mockResolvedValue(preview),
      getOldFavoriteWorkspaceBilibiliExecutionPreflightV1: preflight,
      adoptFavoriteRepositoryLedgerBinding: adopt,
      commandOldFavoriteWorkspaceV1: command
    } as unknown as typeof window.bilimiDesktop
    ...
    await waitFor(() => expect(adopt).toHaveBeenCalledWith('100', {
      ...
      remoteFolderId: 'game-2', remoteTitle: 'bilimi·游戏专区·2'
    }))
    await waitFor(() => expect(refresh).toHaveBeenCalledWith({ reconcileFavoriteBindingProjection: true }))
    await waitFor(() => expect(command).toHaveBeenCalledWith('100', { type: 'confirm-and-execute-bilibili-plan' }))
    expect(refresh.mock.invocationCallOrder[0]).toBeLessThan(command.mock.invocationCallOrder[0])
  ...
  it('replaces an opened backup preflight with the latest target projection before any backup begins', async () => {
    ...
    const openedPreflight = {
      accountMid: '100', workspaceId: 'workspace-100',
      missingLedgers: [{ logicalLedgerId: 'old-target', logicalTitle: 'bilimi·旧目标', reason: 'unbacked' as const }],
      requiredPhysicalShards: []
    }
    const latestPreflight = {
      accountMid: '100', workspaceId: 'workspace-100',
      missingLedgers: [{ logicalLedgerId: 'new-target', logicalTitle: 'bilimi·新目标', reason: 'unbacked' as const }],
      requiredPhysicalShards: []
    }
    const preflight = vi.fn()
      .mockResolvedValueOnce(openedPreflight)
      .mockResolvedValueOnce(latestPreflight)
    const sync = vi.fn().mockResolvedValue({ ok: true })
    const command = vi.fn().mockResolvedValue(preview)
    window.bilimiDesktop = {
      openOldFavoriteWorkspaceV1: vi.fn().mockResolvedValue(preview),
    ...
    const openedConfirmation = await screen.findByRole('dialog', { name: '同步前备册确认' })
    expect(openedConfirmation).toHaveTextContent('收藏夹：bilimi·旧目标（未备册）')
    fireEvent.click(within(openedConfirmation).getByRole('button', { name: '确认备册并继续' }))

    await waitFor(() => expect(screen.getByRole('dialog', { name: '同步前备册确认' })).toHaveTextContent('收藏夹：bilimi·新目标（未备册）'))
    expect(screen.getByRole('dialog', { name: '同步前备册确认' })).not.toHaveTextContent('收藏夹：bilimi·旧目标（未备册）')
    expect(sync).not.toHaveBeenCalled()
    expect(command).not.toHaveBeenCalled()
  })

  it('shows only selected unbacked targets and their post-first-backup shards', async () => {
    const preview = {
      version: 1 as const, accountMid: '100', workspaceId: 'workspace-100', status: 'previewing' as const,
      mode: 'incremental' as const, segmentSize: 2000, hasMultipleSegments: false,
      scan: { phase: 'complete' as const, failureCount: 0 }, continuationCount: 0, sourceFolders: [],
    ...
    const preflight = vi.fn().mockResolvedValue({
      ...
      requiredPhysicalShards: [{ logicalLedgerId: 'game', logicalTitle: 'bilimi·游戏专区', shardNumber: 2, requiredAssignmentCount: 1_208, bindingCandidates: [] }]
    })
    window.bilimiDesktop = {
      openOldFavoriteWorkspaceV1: vi.fn().mockResolvedValue(preview),
      getOldFavoriteWorkspaceBilibiliExecutionPreflightV1: preflight,
    ...
    render(<ControlledFavoriteLedgerPanel currentAccountMid="100" missingLedgerIds={[]}
      ledgers={[
        { id: 'game', displayName: 'bilimi·游戏专区', keywords: [], enabled: true, priority: 10, isDefault: false, bindingState: 'unbacked' },
        { id: 'genshin', displayName: 'bilimi·原神', keywords: [], enabled: true, priority: 20, isDefault: false, bindingState: 'unbacked' }
      ]}
    ...
    const confirmation = await screen.findByRole('dialog', { name: '同步前备册确认' })
    expect(confirmation).toHaveTextContent('收藏夹：bilimi·游戏专区（未备册）')
    expect(confirmation).toHaveTextContent('新增分册：bilimi·游戏专区·2（本轮需容纳 1208 条新归属）')
    const gameLedgerItem = within(confirmation).getByText('收藏夹：bilimi·游戏专区（未备册）').closest('li')
    expect(gameLedgerItem).not.toBeNull()
    expect(within(gameLedgerItem!).getByText('新增分册：bilimi·游戏专区·2（本轮需容纳 1208 条新归属）')).toBeInTheDocument()
    expect(confirmation).not.toHaveTextContent('bilimi·原神')
  })

  it('requires explicit id confirmation before adopting a discovered capacity shard', async () => {
    const preview = {
      version: 1 as const, accountMid: '100', workspaceId: 'workspace-100', status: 'previewing' as const,
      mode: 'incremental' as const, segmentSize: 2000, hasMultipleSegments: false,
      scan: { phase: 'complete' as const, failureCount: 0 }, continuationCount: 0, sourceFolders: [],
    ...
    const candidatePreflight = {
      ...
      requiredPhysicalShards: [{
        logicalLedgerId: 'game', logicalTitle: 'bilimi·游戏专区', shardNumber: 2, requiredAssignmentCount: 1,
        bindingCandidates: [{ remoteFolderId: 'remote-game-2', remoteTitle: 'bilimi·游戏专区·2', memberCount: 6 }]
      }]
    }
    const clearPreflight = { accountMid: '100', workspaceId: 'workspace-100', missingLedgers: [], requiredPhysicalShards: [] }
    const preflight = vi.fn()
    ...
    await waitFor(() => expect(adopt).toHaveBeenCalledWith('100', {
      logicalLedgerId: 'game', logicalTitle: 'bilimi·游戏专区', shardNumber: 2,
      remoteFolderId: 'remote-game-2', remoteTitle: 'bilimi·游戏专区·2'
    }))
    await waitFor(() => expect(command).toHaveBeenCalledWith('100', { type: 'confirm-and-execute-bilibili-plan' }))
    expect(provision).not.toHaveBeenCalled()
  })
  ...
  it('switches a multi-batch guide to the current batch only after the user selects it', async () => {
    const preview = {
      version: 1 as const, accountMid: '100', workspaceId: 'workspace-100', status: 'previewing' as const,
      mode: 'incremental' as const, segmentSize: 500, hasMultipleSegments: true,
      scan: { phase: 'complete' as const, failureCount: 0 }, continuationCount: 0,
  ...
  it('renders UP and tag recommendation cards and restores whole-round choices across segments and remounts', async () => {
    ...
    await openPersistedWorkspaceGuide()
    fireEvent.change(screen.getByRole('combobox', { name: '整理批次' }), { target: { value: 'segment-1' } })
    await waitFor(() => expect(command).toHaveBeenCalledWith('100', { type: 'select-segment', segmentId: 'segment-1' }))
    fireEvent.click(await screen.findByRole('button', { name: '推荐收藏夹' }))
  ...
  it('settles a rapid recommendation adopt then cancel as a disabled local rule when the first save completes late', async () => {
    const preview = {
      version: 1 as const, accountMid: '100', workspaceId: 'workspace-100', status: 'previewing' as const,
      mode: 'incremental' as const, segmentSize: 2000, hasMultipleSegments: false,
      scan: { phase: 'complete' as const, failureCount: 0 }, continuationCount: 0,
      sourceFolders: [], segments: [], currentSegment: null, classifications: {},
      recommendations: {
        candidates: [{ id: 'author-late-cancel', displayName: 'bilimi·迟到取消', kind: 'author' as const, count: 2, reason: '竞态' }],
        adoptedCandidateIds: [] as string[]
      },
      history: { cursor: 0, length: 0 }
    }
    ...
    const saveLedgerEnabled = vi.fn().mockResolvedValue(undefined)
    const remoteCreateOrBind = vi.fn()
    const remoteRename = vi.fn()
    const remoteDelete = vi.fn()
    const remoteUnbind = vi.fn()
    const remoteMoveMembers = vi.fn()
    const remoteVideoSync = vi.fn()
    const command = vi.fn((_accountMid: string, request: { type: string; candidateIds?: string[] }) => Promise.resolve({
      ...preview,
      recommendations: {
        ...preview.recommendations,
        adoptedCandidateIds: request.type === 'set-recommended-candidates' ? request.candidateIds ?? [] : []
      }
    }))
    window.bilimiDesktop = {
      openOldFavoriteWorkspaceV1: vi.fn().mockResolvedValue(preview),
      commandOldFavoriteWorkspaceV1: command,
      provisionOldFavoriteWorkspaceBilibiliExecutionPreflightShardsV1: remoteCreateOrBind,
      repairBoundFavoriteLedgerRemoteTitle: remoteRename,
      deleteManagedRemoteFolders: remoteDelete,
      releaseDefaultFavoriteLedgerBindings: remoteUnbind,
      executeOldFavoriteWorkspaceBilibiliPlanV1: remoteMoveMembers
    } as unknown as typeof window.bilimiDesktop

    const saveLedgers = vi.fn(() => save.promise)
    render(<ControlledFavoriteLedgerPanel currentAccountMid="100" ledgers={[]} missingLedgerIds={[]}
      onEnsureLedgers={vi.fn()} onSaveLedgers={saveLedgers} onSaveLedgerEnabled={saveLedgerEnabled}
      onSyncLedgers={remoteVideoSync} />)
    await openPersistedWorkspaceGuide()
    fireEvent.click(await screen.findByRole('button', { name: '推荐收藏夹' }))

    const candidate = screen.getByRole('checkbox', { name: '迟到取消' })
    fireEvent.click(candidate)
    await waitFor(() => expect(saveLedgers).toHaveBeenCalledOnce())
    fireEvent.click(candidate)

    await act(async () => { save.resolve(undefined); await save.promise })

    await waitFor(() => expect(saveLedgerEnabled).toHaveBeenCalledWith('author-late-cancel', false))
    expect(command).toHaveBeenCalledWith('100', {
      type: 'set-recommended-candidates', candidateIds: []
    })
    expect(remoteCreateOrBind).not.toHaveBeenCalled()
    expect(remoteRename).not.toHaveBeenCalled()
    expect(remoteDelete).not.toHaveBeenCalled()
    expect(remoteUnbind).not.toHaveBeenCalled()
    expect(remoteMoveMembers).not.toHaveBeenCalled()
    expect(remoteVideoSync).not.toHaveBeenCalled()
  })
  ...
  it('marks a promoted recommendation save as recommendation-only so it does not request a full rule reclassification', async () => {
    const preview = {
      ...
      sourceFolders: [], segments: [], currentSegment: null, classifications: {},
      recommendations: {
        candidates: [{ id: 'author-fast', displayName: 'bilimi·快速推荐', kind: 'author' as const, count: 2, reason: 'Fast' }],
        adoptedCandidateIds: [] as string[]
      },
      history: { cursor: 0, length: 0 }
    }
    window.bilimiDesktop = {
      ...
      commandOldFavoriteWorkspaceV1: vi.fn().mockResolvedValue({
        ...preview,
        recommendations: { ...preview.recommendations, adoptedCandidateIds: ['author-fast'] }
      })
    } as unknown as typeof window.bilimiDesktop
    ...
    await openPersistedWorkspaceGuide()
    fireEvent.click(await screen.findByRole('button', { name: '推荐收藏夹' }))
    fireEvent.click(screen.getByRole('checkbox', { name: '快速推荐' }))

    await waitFor(() => expect(save).toHaveBeenCalledWith([
      expect.objectContaining({ id: 'author-fast', displayName: 'bilimi·快速推荐' })
    ], { deleteDisabled: false, recommendationOnly: true }))
  })
  ...
  it('maps a lower recommendation toggle back to its saved top rule by stable id', async () => {
    ...
    await waitFor(() => expect(command).toHaveBeenCalledWith('100', {
      type: 'set-round-excluded-ledger-ids', ledgerIds: ['saved-alice'], mergeFavoriteRuleHistory: true
    }))
  })
  ...
  it('removes a pure recommended local draft card when it is deselected below', async () => {
    const preview = {
      version: 1 as const, accountMid: '100', workspaceId: 'workspace-100', status: 'previewing' as const,
      mode: 'incremental' as const, segmentSize: 2000, hasMultipleSegments: false,
      scan: { phase: 'complete' as const, failureCount: 0 }, continuationCount: 0,
    ...
    render(<ControlledFavoriteLedgerPanel currentAccountMid="100" ledgers={[{
      ...
      priority: 10_000, ruleOrigin: 'recommendation-draft', bindingState: 'unbound', isDefault: false
    }]} missingLedgerIds={[]} onEnsureLedgers={vi.fn()} onSaveLedgers={vi.fn()} />)
    await openPersistedWorkspaceGuide()
    fireEvent.click(await screen.findByRole('button', { name: '推荐收藏夹' }))
  ...
  it('keeps an adopted persisted recommendation rule while deselecting its exact lower candidate', async () => {
    const preview = {
      version: 1 as const, accountMid: '100', workspaceId: 'workspace-100', status: 'previewing' as const,
      mode: 'incremental' as const, segmentSize: 2000, hasMultipleSegments: false,
      scan: { phase: 'complete' as const, failureCount: 0 }, continuationCount: 0,
    ...
    await waitFor(() => expect(saveLedgerEnabled).toHaveBeenCalledWith('author-adopted', false))
    await waitFor(() => expect(command).toHaveBeenCalledWith('100', {
      type: 'set-recommended-candidates', candidateIds: []
    }))
    expect(window.bilimiDesktop?.deleteFavoriteLedgersLocal).toBeUndefined()

    rerender(<ControlledFavoriteLedgerPanel {...props} ledgers={[{ ...initialLedgers[0]!, enabled: false }]} />)
    await waitFor(() => expect(screen.getByRole('button', { name: 'Adopted' })).toBeInTheDocument())
  })

  it('keeps an adopted recommendation after it has been backed to Bilibili when the lower candidate is deselected', async () => {
    const preview = {
      version: 1 as const, accountMid: '100', workspaceId: 'workspace-100', status: 'previewing' as const,
      mode: 'incremental' as const, segmentSize: 2000, hasMultipleSegments: false,
      scan: { phase: 'complete' as const, failureCount: 0 }, continuationCount: 0,
    ...
    await waitFor(() => expect(saveLedgerEnabled).toHaveBeenCalledWith('author-adopted-bound', false))
    await waitFor(() => expect(command).toHaveBeenCalledWith('100', {
      type: 'set-recommended-candidates', candidateIds: []
    }))
  })
  ...
  it('keeps an adopted lower recommendation selected when its local enabled write fails', async () => {
    const preview = {
      version: 1 as const, accountMid: '100', workspaceId: 'workspace-100', status: 'previewing' as const,
      mode: 'incremental' as const, segmentSize: 2000, hasMultipleSegments: false,
      scan: { phase: 'complete' as const, failureCount: 0 }, continuationCount: 0,
    ...
    const saveLedgerEnabled = vi.fn().mockRejectedValue(new Error('local enabled write failed'))
    const onTransientFeedback = vi.fn()
    window.bilimiDesktop = {
      openOldFavoriteWorkspaceV1: vi.fn().mockResolvedValue(preview),
      commandOldFavoriteWorkspaceV1: command
    ...
    render(<ControlledFavoriteLedgerPanel currentAccountMid="100" missingLedgerIds={[]}
      onEnsureLedgers={vi.fn()} onSaveLedgers={vi.fn()} onSaveLedgerEnabled={saveLedgerEnabled} onTransientFeedback={onTransientFeedback}
      ledgers={[{
    ...
    await waitFor(() => expect(saveLedgerEnabled).toHaveBeenCalledWith('author-delete-failed', false))
    expect(screen.getByRole('checkbox', { name: 'Delete failed', checked: true })).toBeInTheDocument()
    await waitFor(() => expect(command).toHaveBeenLastCalledWith('100', expect.objectContaining({
  ...
  it('keeps a saved top rule selected even when its lower recommendation starts unchecked', async () => {
    const preview = {
      version: 1 as const, accountMid: '100', workspaceId: 'workspace-100', status: 'previewing' as const,
      mode: 'incremental' as const, segmentSize: 2000, hasMultipleSegments: false,
      scan: { phase: 'complete' as const, failureCount: 0 }, continuationCount: 0,
    ...
    expect(screen.getByRole('button', { name: '移出同步 bilimi·Bound' })).toBeInTheDocument()
  ...
  it('shows a returned DeepSeek partial failure and retries only failed chunks', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '重试失败 1 条' }))
  ...
  it('keeps executing multi-batch workspaces browseable while locking every mutation', async () => {
    ...
    fireEvent.change(screen.getByRole('combobox', { name: '整理批次' }), { target: { value: 'segment-1' } })
    await waitFor(() => expect(command).toHaveBeenCalledWith('100', { type: 'view-segment', segmentId: 'segment-1' }))
    fireEvent.click(screen.getByRole('button', { name: '推荐收藏夹' }))
