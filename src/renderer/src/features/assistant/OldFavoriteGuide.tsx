...
const ORGANIZING_GUIDE_HINTS: OrganizingGuideHint[] = [
  ...
  { label: '① 扫描概览：', detail: [{ text: '扫描所有视频收藏的基本信息。扫描完成后会补取标签，标签是分类的重要依据，建议耐心等待，不要提前采用；默认扫描到的全部收藏夹参与分类整理，可以取消不想整理的非bilimi收藏夹。' }] },
  { label: '② 推荐收藏夹：', detail: [{ text: '根据扫描到的UP主和高频标签生成推荐收藏夹，可勾选想采用的推荐收藏夹。勾选后的收藏夹会参与整理收藏分类；也可以自建收藏夹，保存并勾选即可参与分类。未备册不影响本轮草稿，整理结束后可再备册并同步到 B 站。' }] },
  { label: '③ 归档预览：', detail: [{ text: '检查分类结果，由于本地分类能力有限，建议用DeepSeek辅助整理未匹配到合适分类和把握不太稳的视频，也可手动调整转移。在上方收藏夹区域编辑或者新增bilimi收藏夹，归档预览会重新计算。' }] },
