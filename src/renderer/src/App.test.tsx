...
describe('App runtime integration', () => {
  ...
  it('does not report a visible backup as successful while its final Bilibili inventory is stale', async () => {
    const accountMid = '100'
    const music = createDefaultFavoriteLedgers().find((ledger) => ledger.id === 'music')!
    const savePreferences = vi.fn(async (preferences: AssistantPreferences) => preferences)
    const adoptFavoriteRepositoryLedgerBinding = vi.fn().mockResolvedValue(undefined)
    ...
    await expect(requestRuntime({
      ...
      options: { backupTargetLedgerIds: [music.id], requireFinalBackupVerification: true }
    })).resolves.toMatchObject({
      ok: false,
    ...
    expect(savedMusic).toMatchObject({ bilibiliFolderId: undefined, bindingState: 'unbacked' })
  })

  it('publishes completed exact bindings while later bindings in a visible backup are still pending', async () => {
    const accountMid = '100'
    const [music, film] = createDefaultFavoriteLedgers().slice(0, 2)
    const [backupMusic, backupFilm] = [music, film].map((ledger) => ({
      ...ledger,
      enabled: true,
      bindingState: 'unbound' as const,
      managedFolderDeletedByUser: true
    }))
    let finishFilmBinding: (() => void) | undefined
    const filmBinding = new Promise<void>((resolve) => { finishFilmBinding = resolve })
    const adoptFavoriteRepositoryLedgerBinding = vi.fn(async (_accountMid: string, input: { logicalLedgerId: string }) => {
      if (input.logicalLedgerId === backupFilm.id) await filmBinding
    })
    const { requestRuntime, requestRuntimeDirect } = renderAppWithRuntimeBridge({
      loadPreferences: vi.fn().mockResolvedValue(createAppPreferences({
        favoriteAccountPreferences: {
          [accountMid]: { defaultFavoriteSystemEnabled: true, favoriteLedgers: [backupMusic, backupFilm] }
        }
      })),
      readBilibiliAccountMid: vi.fn().mockResolvedValue(accountMid),
      adoptFavoriteRepositoryLedgerBinding
      ...
      executeJavaScript: vi.fn(async (script: string, userGesture?: boolean) => {
        ...
        if (script.includes(LEDGER_SAVE_SCRIPT_MARKER)) {
          return {
            ...
            verified: true,
            ledgers: [
              { ...backupMusic, bilibiliFolderId: 'new-music', bilibiliFolderIds: ['new-music'], bindingState: 'bound' as const },
              { ...backupFilm, bilibiliFolderId: 'new-film', bilibiliFolderIds: ['new-film'], bindingState: 'bound' as const }
            ],
            steps: ['api:ledger:create:music', 'api:ledger:create:film'],
            missingTargets: [],
            message: '收藏夹已创建。'
          }
        }
    ...
    await waitFor(() => expect(window.bilimiDesktop.loadPreferences).toHaveBeenCalled())
    await expect(requestRuntime({ id: 'snapshot-before-incremental-binding-progress', type: 'snapshot' }))
      .resolves.toMatchObject({ favoriteLedgerStatus: { verified: true } })
    let operation!: Promise<AssistantRuntimeResponsePayload>
    await act(async () => {
      operation = requestRuntimeDirect({
        id: 'backup-publishes-incremental-binding-progress',
        type: 'save-ledgers',
        ledgers: [backupMusic, backupFilm],
        options: {
          backupTargetLedgerIds: [backupMusic.id, backupFilm.id],
          requireFinalBackupVerification: true
        }
      })
      await vi.waitFor(() => expect(adoptFavoriteRepositoryLedgerBinding).toHaveBeenCalledTimes(2))
    })

    await expect(requestRuntime({ id: 'snapshot-during-second-binding', type: 'snapshot' })).resolves.toMatchObject({
      favoriteLedgerStatus: {
        backupProgress: { targetLedgerIds: [backupMusic.id, backupFilm.id], completedLedgerIds: [backupMusic.id] }
      }
    })
  ...
  it('reports a backup failure when final verified discovery still leaves its target unbound', async () => {
    const accountMid = '100'
    const music = createDefaultFavoriteLedgers().find((ledger) => ledger.id === 'music')!
    const other = {
      ...createDefaultFavoriteLedgers().find((ledger) => ledger.id === 'game')!,
      bilibiliFolderId: 'old-other', bindingState: 'unbound' as const
    }
    const { requestRuntime } = renderAppWithRuntimeBridge({
      loadPreferences: vi.fn().mockResolvedValue(createAppPreferences({
        favoriteAccountPreferences: {
          [accountMid]: { defaultFavoriteSystemEnabled: true, favoriteLedgers: [music, other] }
        }
      })),
      readBilibiliAccountMid: vi.fn().mockResolvedValue(accountMid),
      adoptFavoriteRepositoryLedgerBinding: vi.fn().mockResolvedValue(undefined)
    ...
    Object.assign(webview, {
      executeJavaScript: vi.fn(async (script: string, userGesture?: boolean) => {
        if (userGesture) return { hasUserId: true, hasCsrf: true }
        if (script.includes('已绑定收藏夹改名预检')) return { ok: true, verified: true, observedShards: [] }
        if (script.includes(LEDGER_SAVE_SCRIPT_MARKER)) {
          return {
            ok: true,
            verified: true,
            ledgers: [{ ...music, bindingState: 'unbacked' as const }, other],
            steps: ['api:ledger:create:music'],
            missingTargets: [],
            message: '收藏夹已备册。'
          }
        }
        if (isLedgerStatusScript(script)) {
          return {
            ok: false,
            verified: true,
            ledgers: [{ ...music, bindingState: 'unbound' as const }, other],
            missingLedgerIds: [],
            unboundLedgerIds: [music.id, other.id],
            unboundCandidates: [{ ledgerId: music.id, candidates: [{ id: 'observed-music', title: music.displayName, memberCount: 0 }] }],
            backupConflictLedgerIds: [],
            message: 'B 站目录已核验，但正式绑定尚未完成。'
          }
        }
        return { hasUserId: true, hasCsrf: true }
      })
    ...
    await waitFor(() => expect(window.bilimiDesktop.loadPreferences).toHaveBeenCalled())
    await expect(requestRuntime({
      id: 'backup-final-unbound-target', type: 'save-ledgers', ledgers: [music, other],
      options: { backupTargetLedgerIds: [music.id], requireFinalBackupVerification: true }
    })).resolves.toMatchObject({
      ok: false,
  ...
  it('accepts a final verified backup when only an unrelated historical ledger remains unbound', async () => {
    const accountMid = '100'
    const music = createDefaultFavoriteLedgers().find((ledger) => ledger.id === 'music')!
    const other = {
      ...createDefaultFavoriteLedgers().find((ledger) => ledger.id === 'game')!,
      ...
      executeJavaScript: vi.fn(async (script: string, userGesture?: boolean) => {
        ...
        if (script.includes(LEDGER_SAVE_SCRIPT_MARKER)) {
          return {
            ok: true,
            verified: true,
            ledgers: [{ ...music, bilibiliFolderId: 'bound-music', bilibiliFolderIds: ['bound-music'], bindingState: 'bound' as const }, other],
            steps: ['api:ledger:create:music'],
            missingTargets: [],
            message: '收藏夹已备册。'
          }
    ...
    await expect(requestRuntime({
      id: 'backup-final-historical-unbound-is-out-of-scope', type: 'save-ledgers', ledgers: [music, other],
      options: { backupTargetLedgerIds: [music.id], requireFinalBackupVerification: true }
    })).resolves.toMatchObject({
      ok: true,
      unboundLedgerIds: [],
  ...
  it('does not report backup success before final discovery has been verified', async () => {
    const accountMid = '100'
    const music = createDefaultFavoriteLedgers().find((ledger) => ledger.id === 'music')!
    const { requestRuntime } = renderAppWithRuntimeBridge({
      loadPreferences: vi.fn().mockResolvedValue(createAppPreferences({
      ...
      executeJavaScript: vi.fn(async (script: string, userGesture?: boolean) => {
        ...
        if (script.includes(LEDGER_SAVE_SCRIPT_MARKER)) {
          return {
            ok: true,
            verified: true,
            ledgers: [{ ...music, bindingState: 'unbacked' as const }],
            steps: ['api:ledger:create:music'],
            missingTargets: [],
            message: '收藏夹已备册。'
          }
    ...
    await waitFor(() => expect(window.bilimiDesktop.loadPreferences).toHaveBeenCalled())
    await expect(requestRuntime({
      id: 'backup-final-discovery-unverified', type: 'save-ledgers', ledgers: [music],
      options: { backupTargetLedgerIds: [music.id], requireFinalBackupVerification: true }
    })).resolves.toMatchObject({
      ok: false,
      missingTargets: [music.id],
