...
describe('renderer porcelain theme styles', () => {
  ...
  it('keeps favorite ledger status near the toolbar and ledger copy compact', () => {
    ...
    expectStyleSnippet('.favorite-ledger-panel__pending-queue { display: grid; gap: 8px; border-block: 1px solid rgba(31, 99, 181, 0.16);')
    expectStyleSnippet('.favorite-ledger-panel__pending-metrics { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr));')
    expectStyleSnippet('.favorite-ledger-panel__pending-list article { display: grid; grid-template-columns: minmax(0, 1fr) auto;')
