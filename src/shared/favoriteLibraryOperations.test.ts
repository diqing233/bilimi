...
describe('determineFavoriteOperationEligibility', () => {
  it('classifies managed, unmatched, Bilibili, and mixed scopes deterministically', () => {
    ...
    expect(determineFavoriteOperationEligibility({ source: { kind: 'folder', folderId: 'bilibili:user' }, aids: [5], folders })).toMatchObject({
      sourceScopeKind: 'bilibili-user-folder', allowedActions: ['copy', 'delete-local', 'remove-managed-placement']
    })
    expect(determineFavoriteOperationEligibility({
      ...
      allowedActions: ['copy', 'move', 'delete-local', 'remove-managed-placement']
    })
  })
