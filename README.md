...
- 整理旧收藏时，先生成预览，再由用户确认；有归档任务时，点击确认整理后会二次提醒，开始后本轮整理不可更改，完成后点“好的”结束本轮并重新扫描才能开启下一轮；没有归档任务时，确认页会提示可直接点击“确认整理”结束本轮，不会执行归档请求。
- 已完整整理成功的视频会在后续扫描中显示为“已整理跳过”，只让新增、待分类和上次未完整成功的收藏进入本轮，减少二次整理对原结果的影响。
- 扫描会核对已保护视频的历史 Bilimi 账册归档；部分缺失或全部失效时仍保持保护，并提示用户主动重新整理异常收藏。
...
转写队列支持用户取消；应用重启后会把可恢复的转写草稿先归档，再清空未完成队列。当前范围不包含固定自动超时策略。
...
- DeepSeek API key 保存成功后设置页只显示保存状态，不回显密钥内容；如果读取状态失败，可以重新填写保存。
