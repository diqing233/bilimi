...
截图：`C:\Users\diqing\AppData\Local\Temp\codex-clipboard-f40720ae-4f01-41b4-94ec-d412f60b32b2.png`

截图目标：`整理收藏 → 归档预览 → 本轮总览`。推荐收藏夹预览仍不符合用户预期；本条记录为此前多轮修复后仍未解决的反馈。
...
```text
归档预览有效，但是确认执行里的已汇总不是显示一致吗，这个很简单吧
```
...
| I005 | R006、R007、R008 | 归档预览有效时，确认执行的`本轮总览`必须显示同一版的`已汇总`、已处理/已分类/暂存、每个稳定规则 ID的预计归档及其分批数；禁止确认执行另行用局部状态重算出旧数或 0。 | `整理收藏 → 归档预览 → 本轮总览`；`整理收藏 → 确认执行 → 本轮总览`；主进程`overview.archiveTargets`。 | 多批且切到`本轮总览`时显示；当前批视图仍维持原有单批范围。只有权威投影真实无成员时才显示 0。 | 手动分类、DeepSeek 结果或失败回退、撤销/恢复游标、推荐采用完成后，先更新主进程投影，再让两个页面读快照；不在点击处理器同步扫描所有视频。 | 本轮只读取本地工作区及 Electron UI；未创建、绑定、删除、移动 B 站收藏夹，未写入视频。 | 不改 DeepSeek 行为、转写、删除确认、单个备册入口、真实视频同步，且不以同步全轮视频制造点击卡顿。 | `updateOverviewClassifications`、`captureCurrentSegmentOverviewClassifications`、`createOverviewProjection`、`OldFavoriteWholeRunOverview`及两个步骤组件。 | 已实施；确认页真实 UI 切换待验证 | 代码：`electron/main/oldFavoriteWorkspaceCoordinator.ts:1748,3504,3681,4501,4523,5609,6704,7308,7595`在运行时分类变化后更新投影；`OldFavoriteOverviewControls.tsx:35-80`从单一`overview.archiveTargets`映射展示；`OldFavoriteArchivePreviewStep.tsx:549,652`和`OldFavoriteConfirmationStep.tsx:241,391`均传入同一`snapshot`给该组件。自动化：协调器 355/355；面板、预览、确认与同步/API 聚焦回归 308/308；收藏夹 API 与总览组件 311/311（本轮共 974 项通过）。Electron 只读验收：归档预览显示`已汇总 2/2`、2577/1524/1053，`bilimi·honker233` 34（24+10）、`bilimi·杨颜同学` 9（8+1）；截图`.codex-artifacts/2026-08-28-favorite-round-readonly.png`、`.codex-artifacts/2026-08-28-authoritative-recommendation-archive-projection.png`。本次点击确认执行导航未切换页面，故未将确认页真实 UI 记为已验收；未执行任何 B 站副作用，也未通过真实勾选操作量化鼠标不卡。 |
