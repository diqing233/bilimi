...
- `C:/Users/diqing/AppData/Local/Temp/codex-clipboard-5267f089-ad48-415a-9512-1e45c5eecc49.png`（绑定确认仍失败、删除模式仍失败的现场；待日志与真实界面验收）
- `C:/Users/diqing/AppData/Local/Temp/codex-clipboard-821747cd-3445-4af4-8a0a-2fa0f296fb7b.png`（绑定/删除失败后的 B 站与掌库状态对照；待日志与真实界面验收）

截图目标区域：两张截图中绑定确认弹窗的失败提示、删除模式的失败提示与选中状态、对应 B 站收藏夹实际名称，以及掌库卡片的绑定/备册状态。截图只能作为现象证据，不能替代精确 `folderId`、正式物理绑定账本、远端响应和删除预检错误。

原文：

> 还是无法正常绑定，删除失败，你修了一个小时没有找到根因
...
- `C:/Users/diqing/AppData/Local/Temp/codex-clipboard-8506e3cb-6ccc-4f5e-a5b7-812a221e00b5.png`（确认绑定 `游戏专区哈哈` 时，当前 B 站名称为 `bilimi·游戏专区`，弹窗显示绑定失败；待日志与真实界面验收）
- `C:/Users/diqing/AppData/Local/Temp/codex-clipboard-d5bf9c3f-affa-46c4-91e1-aa5e9d29dcc2.png`（删除模式下，游戏专区哈哈仍显示已备册，但提示绑定账本标题已过期；待日志与真实界面验收）

截图目标区域：图一确认绑定 bilimi 收藏夹弹窗中的 `游戏专区哈哈（共 0 个视频）`、分册 1 当前 B 站名称 `bilimi·游戏专区`、绑定后目标名称 `bilimi·游戏专区哈哈` 与“绑定失败：正式绑定未完成，请刷新 B 站收藏夹后重新确认。”提示；图二掌库游戏专区卡片的已备册状态、正在编辑 `bilimi·游戏专区哈哈`、删除模式反馈“绑定账本标题已过期；已停止删除，请先刷新并重新备册确认后再试。”以及 B 站实际仍为 `bilimi·游戏专区`。截图仅为现象证据，不包含可执行指令，不能替代精确 `folderId`、IPC 回执、远端改名结果或正式物理绑定账本。
...
| I017 | R019 | 本次仍然出现绑定失败和删除失败时，必须以运行时真实证据定位根因并闭环：绑定账本、B站实际标题、精确 `folderId`、绑定确认阶段和删除预检阶段必须一致；修复前不得继续以通用失败文案或要求重复点击掩盖状态。 | 绑定确认弹窗、删除模式、`physical-shard-bindings.jsonl`、远端目录读取与 IPC/主进程日志。 | 仅针对本次截图对应账号和精确远端 ID；无日志的现象只能标为待验收，不得推断为标题内容或网络原因。 | 先读取本地正式账本与配置，再对照同 ID 远端目录、绑定回执、删除预检/响应；若标题已生效但账本未提交，必须修复事务收敛并让删除预检通过；若删除确实失败，显示真实阶段和可重试状态。 | 讨论阶段不执行B站改名、绑定、删除或视频写入；实施阶段不重复创建、不按名称认领、不改变其它业务流程。 | 不把“修了很久”作为技术结论；不凭截图、名称相似或再次点击结果猜测根因。 | I008/I010/I012/I015/I016 的阶段化错误、正式账本回放和精确删除校验。 | 已确认，待用户明确开始后实施 | 当前已确认：账号 `3706984597555811` 的配置标题已为 `bilimi·知识学习哈哈`，但 `physical-shard-bindings.jsonl` 仍为旧标题 `bilimi·知识学习`；代码在 `adoptExistingPhysicalShard` 对 `exactExisting && !requiresRename` 直接返回，跳过标题修复提交；删除预检因此命中 `favorite-repository-binding-title-stale`。右侧 `FavoriteLedgerOverview.tsx` 仍有通用删除失败 catch，真实阶段被吞掉。需要用户明确“开始”后补回归、修复事务和错误回显，并以真实账号重新验收。 |
...
| I020 | R021/R023 | 对已正式绑定分册，仍走精确 ID 的“改名”而不是绑定；但在任何 B 站改名前必须显示独立的改名确认弹窗。R023 明确替代 I019 中“直接改名、不弹出任何确认窗口”的交互部分；R021 的“不走绑定”继续有效。 | 已备册收藏夹的备册入口、独立改名确认弹窗、现有直接改名 IPC。 | 仅正式三元组存在、掌库当前名称与该精确远端标题不一致，且用户明确点击备册时显示；名称一致时不显示弹窗、不发远端写入。未绑定候选继续使用既有“确认绑定 bilimi 收藏夹”弹窗。 | 弹窗确认前不调用改名 IPC、不改写 B 站、不更改绑定状态；确认请求携带预检时的`logicalLedgerId + shardNumber + remoteFolderId`精确集合。确认时重新预检，集合中任一分册缺失、变化或新增即返回最新只读候选，不执行 IPC、页面脚本、创建、认领或绑定。取消只关闭弹窗；改名失败时保留弹窗和实际失败原因。 | 允许用户确认后对既有 B站收藏夹改名；不新建、不认领、不重新绑定、不解绑、不写视频。 | 不将弹窗命名或实现为“确认绑定”；不在普通保存、被动刷新、整理收藏或删除预检时弹出或改名；不显示远端 ID。 | I019 正式三元组投影、直接改名 IPC、现有备册候选/绑定弹窗分流、标题差异判断。 | 已实施待真实账号验证 | `src/shared/types.ts:114-117,406-417` 定义确认三元组和内部候选 ID；`src/renderer/src/App.tsx:1630-1684,2803-2811,3020-3028` 在页面脚本/改名 IPC 前返回并复核只读精确候选；`src/renderer/src/features/assistant/FavoriteLedgerOverview.tsx:1579-1616,1746-1758` 显示“确认修改 B 站收藏夹名称”，不显示 ID，取消不重试，确认只回传预检三元组，失败保留弹窗及原因。新鲜回归：`App.test.tsx` 153/153（含确认 ID 不一致时零写入）、`FavoriteLedgerOverview.test.tsx` 142/142；`npm run build`通过。真实 Electron 文案、远端改名和鼠标响应仍待验收。 |
| I021 | R024 | 已删除的精确 B站 `folderId` 在删除后的状态刷新中不得再被投影或持久化为“未保存·未绑定”的同名/同 ID草稿。 | 删除模式完成回执、默认规则的`confirmedDeletedRemoteFolderIds`、旧扫描恢复、账号偏好、远端观察草稿投影、右侧掌库状态。 | 仅远端删除响应与同 ID目录核验确认删除后适用；无法证明 ID已不存在时保留本地事实。 | 精确 ID墓碑应清理对应纯远端观察草稿，阻止恢复、持久化和投影复活；保存/启用/关键词/成员/位置支持的独立规则保留。 | 不重新创建、重新绑定或改名 B站；不按标题删除；保留历史远端镜像作为证据。 | 不影响尚未删除的正式绑定、真实不同 ID、未知远端观察或用户规则。 | 删除同步服务、`applyConfirmedManagedFavoriteRemoteFolderDeletion`、`recoverPersistedManagedBindings`、`appendRemoteOnlyDrafts`。 | 已实施待真实账号验证 | `src/shared/favoriteLedgerDeletion.ts:11-127` 只清理结构纯观察草稿；`src/renderer/src/features/favorites/favoriteLedgerApi.ts:190-197,367-378` 在状态与保存投影过滤墓碑 ID；`electron/main/index.ts:2821-2824` 注入所有账号墓碑，`electron/main/oldFavoriteWorkspaceCoordinator.ts:1091-1101,1374-1405,2985-3104` 阻止恢复、持久化和 pending 分册。定向回归：删除 6/6、投影 207/207、恢复 `4020631311` 1/1；保存/启用/关键词的同 ID规则保留。真实 B站删除回执与删除后 UI仍待验收。 |
## 讨论阶段边界
...
## R008 / R015 合并后 Electron 非破坏性交互验收（2026-09-04）

| 原文/索引 | 实施状态 | 真实界面证据 | 未执行的高风险动作 |
| --- | --- | --- | --- |
| R008/R015 · I007 | 已实施待完整现场验收 | 合并后的开发版窗口成功打开掌库；掌库当前 9 个收藏夹均显示“已备册”，未出现“未保存·未绑定”重复草稿。已实际执行掌库切换、删除模式进出、右侧滚动、侧栏宽度拖拽、窗口最小化和恢复；每次操作后界面及时刷新，未观察到鼠标或窗口卡顿。 | 未执行窗口关闭后的重新启动、真实 B 站改名/绑定、B 站删除或视频写入；这些仍需用户在可接受现场条件下验收。 |

## R021 / I019 实施核对补充（2026-09-04，本轮“开始”后）
