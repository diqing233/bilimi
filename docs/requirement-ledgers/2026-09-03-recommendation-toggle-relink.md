...
| I011 | 已实施进行中 | 隔离分支 `codex/recommendation-toggle-relink-rework`，根 `main` 的既有未提交改动未触碰；需求账本、项目书、实现与测试属于同一主题。 | `git diff --check`、680 项定向套件、构建均在提交前重新运行；全量为 4275 通过、2 项无关旧失败。 | 合并回本地 `main` 前仍须重新检查根工作树；若脏工作树阻止合并，不覆盖用户改动。 |
