...
| I005 | R006 | 已确认：已勾选且出现在归档预览的“暂存”规则，即使默认不向暂存写视频，尚未备册时仍必须进入统一同步确认的备册范围；“是否写入暂存视频”不能再决定“是否备册”。 | 整理收藏的归档预览、同步预检、统一确认窗口、备册目标计划与暂存规则。 | 暂存已勾选且本轮归档预览存在暂存目标时，未备册/未绑定必须列入同一确认窗口；视频写入仍默认不选。 | `getBilibiliExecutionPreflight` 以含暂存的权威分类构造备册目标，但只有 `includeInbox=true` 才把暂存放进远端写入分配和容量分册；统一窗口仍以默认 `false` 进入，确认后按暂存首册备册，再以默认写入计划执行。 | 自动化未执行 B 站副作用；测试仅用假数据模拟确认、备册结果与执行命令。真实 Electron 仅可只读检查窗口内容，不能点击确认以免创建/绑定。 | 不改 I003、DeepSeek、转写、删除确认或无关收藏夹流程。 | 已保存启用规则、工作区排除集合、归档分类、预检缺口、确认窗口和冻结计划。 | 已实施待完整验证与 Electron 只读验收 | 代码：`electron/main/oldFavoriteWorkspaceCoordinator.ts` 的 `getBilibiliExecutionPreflight`；UI 沿用 `src/renderer/src/features/assistant/ControlledFavoriteLedgerPanel.tsx` 的唯一确认窗口，`OldFavoriteConfirmationStep.tsx:385` 仍默认调用 `onConfirmAndSync(false)`。RED：新增主进程测试先收到 `missingLedgers: []`，证明旧逻辑漏掉暂存。GREEN：`oldFavoriteWorkspaceCoordinator.test.ts` 定向 3/3 通过，覆盖已勾选暂存首册备册、默认无容量分册和上方排除后不备册；`ControlledFavoriteLedgerPanel.test.tsx` 定向 1/1 通过，覆盖同一窗口列出`bilimi·暂存（未备册）`、复选框默认未选、`backupTargetLedgerIds: ['inbox']`及最终命令无 `includeInbox`。完整相关/全量/构建与 Electron 证据待本阶段末尾补充。 |
...
### I005 实施证据补充（2026-08-26）

- **代码位置与行为：** `electron/main/oldFavoriteWorkspaceCoordinator.ts:4419-4461` 的 `getBilibiliExecutionPreflight` 从含暂存回退的权威分类计算备册目标；`assignmentAids` 仍按 `includeInbox` 决定是否写入暂存视频，默认 `false` 时只额外把有实际预览成员的 `inbox` 纳入缺口检查，不产生容量分册。`electron/main/oldFavoriteWorkspaceCoordinator.ts:6445-6455` 继续在冻结/执行阶段排除未勾选暂存。`src/renderer/src/features/assistant/ControlledFavoriteLedgerPanel.tsx` 沿用唯一“同步前备册确认”窗口，复选框只控制暂存视频写入。
- **自动化证据：** RED 测试先复现默认预检遗漏 `bilimi·暂存`；GREEN 聚焦 `oldFavoriteWorkspaceCoordinator.test.ts` 为 336/336，覆盖暂存首册备册、默认不生成容量分册和上方排除后不再备册。`ControlledFavoriteLedgerPanel.test.tsx` 与 `OldFavoriteConfirmationStep.test.tsx` 合计 193/193，覆盖单一窗口列出`bilimi·暂存（未备册）`、复选框默认未选、确认后 `backupTargetLedgerIds: ['inbox']` 且执行命令无 `includeInbox`。全量 `npm test` 为 240/240，`npm run build` 通过，`git diff --check` 通过。
- **B 站副作用边界：** 测试仅使用本地临时仓库、模拟预检和 mock 同步命令；本轮未执行真实 B 站创建、绑定、删除、移动、收藏或视频写入。真实 Electron 的同步确认窗口尚未进行只读打开验收，因当前开发窗口停留在暂存规则编辑状态；确认按钮及任何远端写入仍待后续安全验收。
