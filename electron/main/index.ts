...
function createFloatingSealWindow() {
  ...
  // Windows 閫忔槑绐楀彛澶辨椿鏃?DWM 浼氭妸鍘熺敓甯ф覆鏌撴垚鐧芥潯锛岀Щ鍔ㄧ獥鍙ｅ彲寮哄埗閲嶆柊鍚堟垚銆?
  const disposeWhiteStripFix =
    process.platform === 'win32' ? installFloatingSealWhiteStripFix(seal) : null

  // 婧愬ご淇細鍓ユ帀 WS_CAPTION锛岃 DWM 涓嶈繘鍏?inactive frame 缁樺埗璺緞锛屽苟鎶?
  // DWMWA_NCRENDERING_POLICY 璁句负 DWMNCRP_DISABLED 浣滅旱娣遍槻寰°�?
  // 鏈獥鍙ｅ凡鏃犱换浣曚緷璧栨爣棰樻爮鐨勫姛鑳斤紙resizable/hasShadow/min/max/thickFrame 鍏ㄥ叧锛夛紝
  // 鍓ュ畠鍦ㄥ姛鑳戒笂闆舵崯澶便�俷udge 浠嶅厹搴曠洿鍒扮‘璁ょǔ瀹氥�?
  if (process.platform === 'win32') {
    installFloatingSealCaptionStrip(seal, {
      // node:child_process spawn 鐨勫閲嶈浇鍦ㄦ垜浠殑绐勬帴鍙ｄ笅涓嶇洿鎺ュ懡涓紝鍋氫竴娆℃樉寮忔摝闄ゃ�?
      spawn: spawn as unknown as NonNullable<
        Parameters<typeof installFloatingSealCaptionStrip>[1]
      >['spawn'],
      // 鏆傛椂鎶婃棩蹇楁帴鍒?console锛屾柟渚垮疄娴嬫湡纭 PS 璋冪敤鐪熺殑鍦ㄨ窇銆佹湁鏃?spawn 閿欒銆?
      // 绋冲畾鍚庡啀鎹㈠洖 noop銆?
