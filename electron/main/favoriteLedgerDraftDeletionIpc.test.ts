...
describe('favorite ledger draft deletion narrow IPC', () => {
  ...
  it('persists only a verified unsaved draft and refreshes preferences without repository or remote calls', () => {
    ...
    expect(handler).not.toContain('removePureRecommendationLedgerDraft')
    expect(handler).not.toContain('recommendation-draft')
  ...
  it('removes selected custom configurations locally and reuses managed-folder cleanup without a B站 delete', () => {
    ...
    expect(handler).not.toContain('markFavoriteLedgerRemoteDraftRediscoveryPending')
    expect(handler).toContain('saveFavoriteAccountPreferences(getDesktopStore(), accountMid, {')
