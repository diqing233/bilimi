...
type FavoriteConfigurationUpdate = {
  workspaceId: string
  version: number
...
const FAVORITE_CONFIGURATION_RECLASSIFICATION_SUPERSEDED = 'Old favorite workspace favorite configuration reclassification superseded.'
...
export class OldFavoriteWorkspaceCoordinator {
  ...
  /** A transient, authoritative write barrier while a saved-rule change is reclassified in the background. */
  private readonly favoriteConfigurationUpdates = new Map<string, FavoriteConfigurationUpdate>()
  private readonly favoriteConfigurationUpdateRuns = new Map<string, Promise<void>>()
  ...
  constructor(private readonly options: {
    ...
    /** Restores only exact deleted local rule snapshots needed by a history position; never performs a remote action. */
    restoreDeletedFavoriteLedgerRulesForHistory?: (accountMid: string, logicalLedgerIds: string[]) => Promise<string[]>
  ...
  resetAfterFullLocalDataClear(): void {
    ...
    this.favoriteConfigurationUpdates.clear()
    this.favoriteConfigurationUpdateRuns.clear()
  }
  ...
  /** Reprojects scanned source relationships after a local backup or binding change. */
  async refreshRelationshipProjection(accountMid: string): Promise<OldFavoriteWorkspaceSnapshot | OldFavoriteWorkspaceRecoveryRequired | null> {
    return this.queue(async () => {
      const workspace = await this.openUnsafe(accountMid)
      if (!workspace || isRecoveryRequired(workspace)) return workspace
      const repository = await this.options.repository.getSnapshot(workspace.accountMid)
      await this.refreshRelationshipProjectionUnsafe(workspace, repository)
      return this.createSnapshotWithExecutionProgress(workspace)
    })
  }
  ...
  private assertFavoriteConfigurationExecutionReady(workspace: OldFavoriteWorkspace) {
    const update = this.favoriteConfigurationUpdates.get(workspace.accountMid)
    if (!update || update.workspaceId !== workspace.id) return
    if (update.status === 'running') throw new Error('Old favorite workspace is updating favorite rules before saving or syncing.')
    throw new Error('Old favorite workspace favorite rule update failed; retry the rule change before saving or syncing.')
  }
  ...
  /**
   * Starts a full configuration reclassification without holding the main
   * command queue. The returned snapshot is the authority that prevents a
   * save or sync from using the old rule projection while work continues.
   */
  async scheduleFavoriteConfigurationReclassification(accountMid: string): Promise<OldFavoriteWorkspaceSnapshot> {
    const scheduled = await this.queue(async () => {
      const workspace = await this.requireWorkspace(accountMid)
      if (workspace.status !== 'previewing') return { workspace: null, version: 0, snapshot: this.createSnapshot(workspace) }
      const version = (this.favoriteConfigurationUpdates.get(workspace.accountMid)?.version ?? 0) + 1
      this.favoriteConfigurationUpdates.set(workspace.accountMid, {
        workspaceId: workspace.id,
        version,
        status: 'running'
      })
      return { workspace: clone(workspace), version, snapshot: this.createSnapshot(workspace) }
    })
    if (scheduled.workspace && scheduled.version > 0) {
      const run = this.runScheduledFavoriteConfigurationReclassification(scheduled.workspace, scheduled.version)
      this.favoriteConfigurationUpdateRuns.set(scheduled.workspace.accountMid, run)
      void run.finally(() => {
        if (this.favoriteConfigurationUpdateRuns.get(scheduled.workspace!.accountMid) === run) {
          this.favoriteConfigurationUpdateRuns.delete(scheduled.workspace!.accountMid)
        }
      })
    }
  ...
  /** Waits only for the latest background rule projection; useful to safely close local test/workspace scopes. */
  async waitForFavoriteConfigurationReclassification(accountMid: string): Promise<void> {
    await this.favoriteConfigurationUpdateRuns.get(accountMid)
  }

  private isFavoriteConfigurationUpdateCurrent(workspace: OldFavoriteWorkspace, version: number) {
    const update = this.favoriteConfigurationUpdates.get(workspace.accountMid)
    return update?.workspaceId === workspace.id && update.version === version && update.status === 'running'
  }

  private assertFavoriteConfigurationUpdateCurrent(workspace: OldFavoriteWorkspace, version: number) {
    if (!this.isFavoriteConfigurationUpdateCurrent(workspace, version)) {
      throw new Error(FAVORITE_CONFIGURATION_RECLASSIFICATION_SUPERSEDED)
    }
  }

  private async runScheduledFavoriteConfigurationReclassification(workspace: OldFavoriteWorkspace, version: number) {
    const stillCurrent = () => this.isFavoriteConfigurationUpdateCurrent(workspace, version)
    try {
      await this.yieldToEventLoop()
      this.assertFavoriteConfigurationUpdateCurrent(workspace, version)
      const reclassified = await this.autoClassifyAllSegmentsUnsafe(workspace, true, true, true, undefined, stillCurrent)
      this.assertFavoriteConfigurationUpdateCurrent(workspace, version)
      await this.yieldToEventLoop()
      await this.queue(async () => {
        this.assertFavoriteConfigurationUpdateCurrent(workspace, version)
        await this.advanceRecoveryBaselineForAcceptedFavoriteConfiguration(reclassified)
        this.assertFavoriteConfigurationUpdateCurrent(workspace, version)
        this.favoriteConfigurationUpdates.delete(workspace.accountMid)
      })
    } catch (error) {
      if (error instanceof Error && error.message === FAVORITE_CONFIGURATION_RECLASSIFICATION_SUPERSEDED) return
      await this.queue(async () => {
        if (!this.isFavoriteConfigurationUpdateCurrent(workspace, version)) return
        this.favoriteConfigurationUpdates.set(workspace.accountMid, {
          workspaceId: workspace.id,
          version,
          status: 'failed',
          ...(error instanceof Error && error.message ? { failureDetail: error.message } : {})
        })
      })
    }
  }
  ...
  private async autoClassifySegmentsUnsafe(
    ...
    const assertCurrent = () => {
      if (shouldContinue && !shouldContinue()) throw new Error(FAVORITE_CONFIGURATION_RECLASSIFICATION_SUPERSEDED)
    }
    assertCurrent()
      ...
      for (const segmentId of segmentIds) {
        ...
        const classifications = classifyMany
          ? await classifyMany(candidates.map(clone), clone(recommendedLedgers), workspace.accountMid, {
              excludedRecommendedLedgers: clone(excludedRecommendedLedgers),
              shouldCancel: () => Boolean(shouldContinue && !shouldContinue())
            })
          : await Promise.all(candidates.map((item) => recommendedLedgers.length
  ...
  async saveCurrentSegmentToLocalLibrary(accountMid: string): Promise<OldFavoriteWorkspace> {
    return this.queue(async () => {
      const workspace = await this.requireWorkspace(accountMid)
      this.assertFavoriteConfigurationExecutionReady(workspace)
  ...
  async acceptCurrentTags(accountMid: string) {
    ...
    return this.queue(async () => {
      ...
      for (const segmentId of this.tagEnrichmentSegmentIds(workspace)) {
        const tagVersion = next.tagVersionsBySegment[segmentId] ?? 0
        const acceptedTagVersion = next.acceptedTagVersionsBySegment[segmentId] ?? 0
        const alreadyAcceptedAtCurrentVersion = next.acceptedSegmentIds.includes(segmentId) &&
          hasTagVersion(next.acceptedTagVersionsBySegment, segmentId) && tagVersion === acceptedTagVersion
        if (alreadyAcceptedAtCurrentVersion) continue
        const acceptedSegmentIds = [...new Set([...next.acceptedSegmentIds, segmentId])]
        const acceptedTagVersionsBySegment = { ...next.acceptedTagVersionsBySegment, [segmentId]: tagVersion }
        const candidate: TagEnrichment = { ...next, acceptedSegmentIds, acceptedTagVersionsBySegment }
  ...
  async getPendingTagEnrichmentAids(accountMid: string) {
    return this.queue(async () => {
      const workspace = await this.requireWorkspace(accountMid)
      return this.getPendingTagEnrichmentAidsUnsafe(workspace)
    })
  }

  async claimNextPendingTagEnrichmentAid(accountMid: string) {
    return this.queue(async () => {
      const workspace = await this.requireWorkspace(accountMid)
      const pending = await this.getPendingTagEnrichmentAidsUnsafe(workspace)
      const inFlight = this.inFlightTagEnrichmentAids.get(workspace.accountMid) ?? new Set<number>()
      const aid = pending.find((candidate) => !inFlight.has(candidate))
      if (aid === undefined) return undefined
      inFlight.add(aid)
      this.inFlightTagEnrichmentAids.set(workspace.accountMid, inFlight)
      return aid
    })
  }

  async releaseClaimedTagEnrichmentAid(accountMid: string, aid: number) {
    return this.queue(async () => {
      const workspace = await this.requireWorkspace(accountMid)
      this.releaseClaimedTagEnrichmentAidUnsafe(workspace.accountMid, aid)
    })
  }

  private async getPendingTagEnrichmentAidsUnsafe(workspace: OldFavoriteWorkspace) {
    const enrichment = this.tagEnrichments.get(workspace.accountMid)
  ...
  async recordTagEnrichment(accountMid: string, aid: number, tags: string[], expectedWorkspaceId?: string) {
    ...
    const recorded = await this.queue(async () => {
      ...
      const enrichment = this.tagEnrichments.get(workspace.accountMid)
      const claimedInFlight = this.inFlightTagEnrichmentAids.get(workspace.accountMid)?.has(aid) === true
      const acceptingInFlightResult = claimedInFlight && (enrichment?.status === 'paused' || enrichment?.status === 'accepted')
      if (!enrichment || (!acceptingInFlightResult && enrichment.status !== 'running') || !enrichment.pendingAids.includes(aid)) return false
      const segmentId = await this.findTagEnrichmentSegmentId(workspace, aid)
      if (!segmentId) return false
      const acceptedSegment = enrichment.acceptedSegmentIds.includes(segmentId)
      if (acceptedSegment && !acceptingInFlightResult) return false
      this.releaseClaimedTagEnrichmentAidUnsafe(workspace.accountMid, aid)
      ...
      const acceptedSegmentIds = acceptedSegment && tagChanged
      ...
      const activePendingAids = pendingAids.filter((pendingAid) => !workspace.segments.some((segment) =>
        acceptedSegmentIds.includes(segment.id) && segment.aids.includes(pendingAid)))
      const next: TagEnrichment = {
        ...enrichment,
        acceptedSegmentIds,
        pendingAids,
        ...
        status: enrichment.status === 'paused' || enrichment.status === 'accepted'
          ? enrichment.status
  ...
  async recordTagEnrichmentFailure(accountMid: string, aid: number, _reason: string, expectedWorkspaceId?: string) {
    ...
    const recorded = await this.queue(async () => {
      ...
      const segmentId = await this.findTagEnrichmentSegmentId(workspace, aid)
      if (!segmentId) return false
      if (enrichment.acceptedSegmentIds.includes(segmentId) && !acceptingInFlightResult) return false
      this.releaseClaimedTagEnrichmentAidUnsafe(workspace.accountMid, aid)
      const pendingAids = enrichment.pendingAids.filter((candidate) => candidate !== aid)
      readyAccountMid = workspace.accountMid
      readySegmentIds = this.newlyReadySegmentIds(workspace, enrichment.pendingAids, pendingAids)
  ...
  /** Resolves a result's batch even after restart only restored the active batch. */
  private async findTagEnrichmentSegmentId(workspace: OldFavoriteWorkspace, aid: number) {
    const loaded = workspace.segments.find((segment) => segment.aids.includes(aid))
    if (loaded) return loaded.id
    const descriptors = this.segmentDescriptors.get(workspace.accountMid) ??
      workspace.segments.map(({ id, index, aids }) => ({ id, index, itemCount: aids.length }))
    const overviewRuntime = this.overviewRuntimes.get(workspace.accountMid)
    const summarized = descriptors.find((descriptor) => overviewRuntime?.segmentAidsBySegment.get(descriptor.id)?.has(aid))
    if (summarized) return summarized.id
    for (const descriptor of descriptors) {
      const segment = await this.options.workspaceStore.loadSegment(workspace.accountMid, workspace.id, descriptor.id)
      if (segment.aids.includes(aid)) return segment.id
    }
    return undefined
  }

  private hasUnacceptedPendingTagEnrichment(
    workspace: OldFavoriteWorkspace,
    enrichment: TagEnrichment,
    acceptedSegmentIds = enrichment.acceptedSegmentIds
  ) {
    const accepted = new Set(acceptedSegmentIds)
    const pendingAids = new Set(enrichment.pendingAids)
    const loadedSegmentsById = new Map(workspace.segments.map((segment) => [segment.id, segment]))
    const overviewRuntime = this.overviewRuntimes.get(workspace.accountMid)
    const descriptors = this.segmentDescriptors.get(workspace.accountMid) ??
      workspace.segments.map(({ id, index, aids }) => ({ id, index, itemCount: aids.length }))
    return descriptors.some((descriptor) => {
      if (accepted.has(descriptor.id)) return false
      const aids = loadedSegmentsById.get(descriptor.id)?.aids ?? overviewRuntime?.segmentAidsBySegment.get(descriptor.id)
      if (aids) return [...aids].some((aid) => pendingAids.has(aid))
      const range = overviewRuntime?.aidRangeBySegment.get(descriptor.id)
      return range?.firstAid !== undefined && range.lastAid !== undefined &&
        [...pendingAids].some((aid) => aid >= range.firstAid! && aid <= range.lastAid!)
    })
  }

  private tagEnrichmentSegmentIds(workspace: OldFavoriteWorkspace) {
    return (this.segmentDescriptors.get(workspace.accountMid) ??
      workspace.segments.map(({ id, index, aids }) => ({ id, index, itemCount: aids.length })))
      .map((segment) => segment.id)
  }
  ...
  private assertWholeRunTagCutoffAccepted(workspace: OldFavoriteWorkspace) {
    const scan = this.scanOverviews.get(workspace.accountMid)?.scan
    if (scan && scan.phase !== 'complete') throw new Error('Old favorite workspace is not ready for local saving.')
    if (!scan && workspace.status === 'scanning') throw new Error('Old favorite workspace is not ready for local saving.')
    if (!this.hasWholeRunTagCutoffAccepted(workspace)) {
      throw new Error('Old favorite workspace whole-run tag enrichment is not complete.')
    }
  }

  private releaseClaimedTagEnrichmentAidUnsafe(accountMid: string, aid: number) {
    const inFlight = this.inFlightTagEnrichmentAids.get(accountMid)
    if (!inFlight) return
    inFlight.delete(aid)
  ...
  async moveHistoryCursor(accountMid: string, targetCursor: number): Promise<OldFavoriteWorkspace> {
    return this.queue(async () => {
      ...
      if (updated === workspace) return clone(workspace)
      const referencedLedgerIds = [...new Set(Object.values(updated.classifications)
        .flatMap((classification) => classification.targetLedgerIds)
        .map((ledgerId) => ledgerId.trim())
        .filter((ledgerId) => ledgerId && ledgerId !== 'inbox'))].sort()
      const restoredLedgerIds = referencedLedgerIds.length
        ? await this.options.restoreDeletedFavoriteLedgerRulesForHistory?.(updated.accountMid, referencedLedgerIds) ?? []
        : []
      const priorExcludedLedgerIds = this.roundExcludedLedgerIdsByAccount.get(updated.accountMid) ?? []
      const restoredSet = new Set(restoredLedgerIds.map((ledgerId) => ledgerId.trim()).filter(Boolean))
      const excludedLedgerIds = restoredSet.size
        ? priorExcludedLedgerIds.filter((ledgerId) => !restoredSet.has(ledgerId))
        : priorExcludedLedgerIds
      ...
      await this.options.workspaceStore.appendOverlay(updated.accountMid, updated.id, {
        ...
        planReadiness: readiness,
        ...(restoredSet.size ? { excludedLedgerIds } : {})
      })
      this.planReadiness.set(updated.accountMid, readiness)
      if (restoredSet.size) this.roundExcludedLedgerIdsByAccount.set(updated.accountMid, excludedLedgerIds)
      const overviewEntries = updated.history.slice(0, updated.historyCursor).map((entry) => ({
        segmentId: this.currentSegment(updated), entry
      }))
      this.updateOverviewClassifications(updated.accountMid, overviewEntries)
      this.workspaces.set(updated.accountMid, updated)
      return clone(updated)
    })
  }
  ...
  private async refreshRelationshipProjectionUnsafe(
    workspace: OldFavoriteWorkspace,
    repository: Awaited<ReturnType<FavoriteRepositoryService['getSnapshot']>>
  ) {
    const overview = this.scanOverviews.get(workspace.accountMid)
    ...
    const localWorkspaceFolders = this.projectLocalWorkspaceFolders(repository)
    const authority = this.inventoryMetrics.get(workspace.accountMid)?.authority ??
      (overview.scan.phase === 'complete' ? 'complete' as const : 'incomplete' as const)
  ...
  private createSnapshot(workspace: OldFavoriteWorkspace): OldFavoriteWorkspaceSnapshot {
    ...
    const currentSegmentCanContinueTagEnrichment = Boolean(currentSegmentId && tagEnrichment &&
      tagEnrichment.status === 'paused' && currentSegmentHasAcceptedTagVersion &&
      currentTagVersion === currentAcceptedTagVersion && !currentSegmentHasUnacceptedTagChanges &&
      this.hasUnacceptedPendingTagEnrichment(workspace, tagEnrichment))
      ...
      (this.segmentDescriptors.get(workspace.accountMid) ?? workspace.segments.map(({ id, index, aids }) => ({ id, index, itemCount: aids.length })))
        .map((segment) => ({
          ...
          ...(() => {
            ...
            const pendingTagItemCount = knownSegmentAids
    ...
    return {
        ...
        tagEnrichment: (() => {
          ...
          const createTagScope = (aids: readonly number[], isCurrentSegment = false) => {
            ...
            const pendingItemCount = aids.reduce((count, aid) =>
              count + Number(pendingTagAids.has(aid)), 0)
      ...
      ...(this.favoriteConfigurationUpdates.get(workspace.accountMid)?.workspaceId === workspace.id ? {
        configurationUpdate: (() => {
          const update = this.favoriteConfigurationUpdates.get(workspace.accountMid)!
          return { version: update.version, status: update.status }
        })()
      } : {}),
      history: {
